...
from Helper import disp, calc_LCS
import resource
...
class Corpus:
	...
	def Calculate_PairWiseFeature(self):
		...
		disp("Creating Pairwise Feature")
		k1 = 1.2
		b  = 0.75
		...
		#for itemID in range(10):
			...
			for question in self.QPerItem[itemID]:
				for sent in self.SPerItem[itemID]:
					...
					'''if bm25 == 0:
																					print "BM25+: "+str(bm25_plus)
																					print "ItemId: "+str(self.QAnswers[question].itemId)+ " Question: "+self.QAnswers[question].question
																					print "ItemId: "+str(self.Sentences[sent].itemId)+ " Sentence: "+self.Sentences[sent].sent'''
					self.PairWiseFeature[(question,sent)] = np.array([[bm25, bm25_plus, LCS]], dtype =np.float64)
			print "Done for item %d"%(itemID)

			accum = self.PairWiseFeature.values()
			accum = sum(accum)
			for key, value in self.PairWiseFeature.items():
				self.PairWiseFeature[key] = value/accum
